// backend/server.js — Jupiter v6 with debug logging
...
async function fetchJupiterPrice(mint) {
  ...
  console.log("📩 Jupiter response:", JSON.stringify(j));
...
// Root
app.get("/", (_req, res) => {
  res.type("text").send("THE ORGANISM API (debug Jupiter) — use /health");
});
